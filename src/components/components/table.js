...
class TableComponent extends Component {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      searchActive: false,
      addButtonLink: this.props.addButtonLink || ''
    }
  ...
  componentWillReceiveProps (nextProps) {
    ...
    let searchByList = this.state.searchByList.reduce((prev, item) => {
      if (item.key === 'Any') return
      return {...prev, [item.key]: item.key}
    }, {})

    data.map(obj => {
      for (let value in searchByList) {
    ...
    for (let i = this.state.offset; i < myLimit; i++) { limitFilter.push(searchFilter[i]) }
  ...
  onOpenConfirmDialog () { this.setState({openConfirmDialog: true}) }
  onCloseConfirmDialog () { this.setState({openConfirmDialog: false}) }
  ...
  onSearchListener (event) {
    ...
    this.setState({search: event.target.value, selectedIndexes: []})
    this.timeout = window.setTimeout(() => { this.getData() }, this.state.searchActive ? 0 : 1000)
    this.setState({searchActive: false})
  }

  onChangeRowsPerPage (event) { if (this.state.limit !== event.target.textContent) { this.setState({limit: parseInt(event.target.textContent), offset: 0}, () => { this.getData() }) } }
  onClickNextPage () { if (this.state.offset + this.state.limit < this.state.iTotalRecords) { this.setState({offset: parseInt(this.state.offset) + parseInt(this.state.limit)}, () => { this.getData() }) } }
  ...
  fnCloseTextSearchBtn () {
    return (this.state.search)
      ? <IconButton className="table-com__search-text-btn-close"
        onClick={() => {
            ...
            <div className={!this.state.showSearch ? 'table-com__search-wrap' : 'd-none table-com__search-wrap'}>
              ...
              <div className="table-com__tools">
                <IconButton
                 ...
                 <LinkNav to={this.state.addButtonLink}>
                  <IconButton
